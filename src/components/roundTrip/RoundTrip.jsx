...
export const RoundTrip = () => {
  ...
  const filterFligth = async(dataForm)=>{
    ...
    delete dataForm1.dateVueloRegreso
    const dataForm2 = { ...dataForm }
    delete dataForm2.dateVueloRegreso
    dataForm2['dateVuelo'] = dataForm.dateVueloRegreso
    dataForm2['origen'] = dataForm.destino
    dataForm2['destino'] = dataForm.origen
    console.log(dataForm2);
    console.log(dataForm1)
    const filteredFlights1 = await searchFligths(dataForm1)
    const filteredFlights2 = await searchFligths(dataForm2)
    
    if (filteredFlights1.length && filteredFlights2.length) {
      console.log(filteredFlights1);
      console.log(filteredFlights2)
      sessionStorage.setItem('vuelosIda', JSON.stringify(filteredFlights1))
      sessionStorage.setItem('vuelosReg', JSON.stringify(filteredFlights2))
      navigate('/flights')
    }else{
      Swal.fire(
        'upps',
...
<div className='container d-flex justify-content-center align-items-center vh-100'>
      ...
      <div className='bg-white d-flex col-md-6 justify-content-between p-2'>
        <Link to='/' className='text-decoration-none btn btn-primary'>Viaje redondo</Link>
        <Link to='oneWay' className='text-decoration-none btn btn-outline-primary'>Viaje sencillo</Link>
      </div>
      <form className='d-flex flex-column gap-3' onSubmit={handleSubmit}>
        ...
        <div className="input-group mb-3">
        <span className="input-group-text" id="basic-addon1">Regreso</span>
        <input type="date" name='dateVueloRegreso' className="form-control" aria-label="Fecha" aria-describedby="date" min={currentDate} onChange={handleChangeInput}/>
        </div>
        </div>
        <div className='col-md-12 d-flex gap-3'>
